...
const unSecuredLoanDataAccess= require("../../dal/liabilities/unSecuredLoan.dal")
...
exports.storeLoan = async (req,res) => {
  const user = req.token_data._id
    const {loanProvider,loan_Number,loan_Id_Number,current_Outstanding_Amount,description} = req.body;
    if (!loanProvider || !loan_Number || !loan_Id_Number || !current_Outstanding_Amount || !description) {
    ...
    const storedLoan = await unSecuredLoanDataAccess.storeLoan(data);
    if (storedLoan){
    return {
      error: false,
      sucess: true,
...
exports.getLoanDetails = async (req, res) => {
  const user = req.token_data._id
  const users = await unSecuredLoanDataAccess.findLoan({user_id:user});
